(function ($) {
  jQuery.fn.treeSelector = function (tree, defaultValues, onChange, params) {
    ...
    var appendSelectedItems = function ($selector, values) {
      if ($selector && values && Array.isArray(values)) {
        ...
        for (var k = 0; k < values.length; k++) {
          ...
          if (item) {
            item.prev('input[type=radio]').prop('checked', true)
     ...
     */
    var uncheckParent = function (inputradio) {
      var closeUl = $(inputradio).closest('ul')
      if (closeUl && closeUl.length) {
        var radio = closeUl.prev('.treeSelector-li-title-box')
          .find('input[type=radio]:first')
        radio.prop('checked', false)
        uncheckParent(radio.get(0))
      }
    }
    ...
    var bindEvents = function ($selector) {
      $selector.on('change', 'input[type=radio]', function (e) {
        if (options.disabled) {
          return false
        }
        if (options.checkWithParent) {
          ...
          if (e.target.checked) {
            if (childrenBox && childrenBox.length > 0) {
              childrenBox.find('input[type=radio]')
...
(function ($) {
  jQuery.fn.treeSelectorCheckbox = function (tree, defaultValues, onChange, params) {
    // use namespace for autoclose
    $(window).off('click.treeSelector').on('click.treeSelector', function (e) {
      var isClickSelector = $(e.target).closest('.treeSelector-container').length > 0
      if (!isClickSelector) {
